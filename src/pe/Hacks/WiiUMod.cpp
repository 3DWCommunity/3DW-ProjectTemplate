...
namespace pe {
    ...
    static void createSceneResourceHeapForTitleSceneHook9(const char* stageName) { // title wants more
        al::createSceneHeap("SingleModeOceanPhase0Stage"); // random entry with 200mb
    }

    static sead::FrameHeap* createCourseSelectResourceHeapHook(size_t size, const sead::SafeString& name, sead::Heap* parent, size_t smth, sead::FrameHeap::HeapDirection dir, bool smth2) { // increase course select resource heap size (ive LICHAARALLEY got no clue why it wants more now.)
    ...
    void initWiiUModHooks() {
        ...
        hk::hook::a64::assemble<"nop">()
            .installAtMainOffset(0x003f80e4) // disable player params being set to new
            .installAtMainOffset(0x003f8184) // "
            .installAtMainOffset(0x003fcb28) // start with boot state instead of top menu state
            .installAtMainOffset(0x003fe798);
